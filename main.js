document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('showDetailsButton').classList.add('hidden');
    updateDeviceTime();
    loadNotes();
});
...
function showDetails() {
    ...
    const showButton = document.getElementById('showDetailsButton');
    const hideButton = document.getElementById('hideDetailsButton');
...
function updateDeviceTime() {
    setInterval(() => {
        ...
        const frames = String(Math.floor((now.getMilliseconds() / 1000) * fps)).padStart(2, '0');
        document.getElementById('deviceTime').textContent = `${hours}:${minutes}:${seconds}:${frames}`;
    }, 1000 / fps);
}
...
function addNoteToTable(noteObj) {
    ...
    locationCell.style.display = 'none'; // Always hide the location cell
...
function editNote(row) {
    const oldTimestamp = row.cells[0].textContent;
    const oldNote = row.cells[1].textContent;
    const oldLocation = row.cells[2].textContent;
    row.cells[0].innerHTML = `<input type="text" value="${oldTimestamp}">`;
    row.cells[1].innerHTML = `<textarea rows="4" cols="50">${oldNote}</textarea>`;
    row.cells[2].textContent = oldLocation;
    row.cells[2].style.display = 'none'; // Always hide the location cell
    row.cells[3].innerHTML = `<button onclick="saveEditedNote(this.parentNode.parentNode, '${oldLocation}')">Save</button>`;
}

function saveEditedNote(row, oldLocation) {
    const newTimestamp = row.cells[0].querySelector('input').value;
    const newNote = row.cells[1].querySelector('textarea').value;
    updateNoteInData(newTimestamp, newNote, oldLocation);
    row.cells[0].textContent = newTimestamp;
...
function updateNoteInData(timestamp, note, location) {
    const index = notesData.findIndex(n => n.timestamp === timestamp && n.location === location);
    if (index !== -1) {
        notesData[index].note = note;
        notesData[index].timestamp = timestamp;
        localStorage.setItem('notesData', JSON.stringify(notesData));
    }
}
...
function clearAll() {
    if (confirm('Are you sure you want to clear all notes? This action cannot be undone.')) {
        ...
        document.getElementById('notesTable').querySelector('tbody').innerHTML = '';
...
function exportTableToCSV() {
    var projectName = document.getElementById("projectName").value.trim() || "Default";
    ...
    for (var i = 1; i < rows.length; i++) {
        ...
        var rowArray = Array.from(cols).map(col => `"${col.innerText.replace(/"/g, '""')}"`);
